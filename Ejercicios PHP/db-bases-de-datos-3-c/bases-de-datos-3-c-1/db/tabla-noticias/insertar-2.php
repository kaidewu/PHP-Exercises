...
if (mb_strlen($categoria, "UTF-8") > $cfg["dbNoticiasTamCategoria"]) {
    print "    <p class=\"aviso\">El categoria no puede tener más de $cfg[dbNoticiasTamCategoria] caracteres.</p>\n";
    print "\n";
} else {
    $categoriaOk = true;
}
...
if (mb_strlen($titulo, "UTF-8") > $cfg["dbNoticiasTamTitulo"]) {
    print "    <p class=\"aviso\">Los titulo no pueden tener más de $cfg[dbNoticiasTamTitulo] caracteres.</p>\n";
    print "\n";
} else {
    $tituloOk = true;
}
...
if (mb_strlen($cuerpo, "UTF-8") > $cfg["dbNoticiasTamCuerpo"]) {
    print "    <p class=\"aviso\">El teléfono no puede tener más de $cfg[dbNoticiasTamCuerpo] caracteres.</p>\n";
    print "\n";
} else {
    $cuerpoOk = true;
}

if (mb_strlen($creado, "UTF-8") > $cfg["dbNoticiasTamCreado"]) {
    print "    <p class=\"aviso\">El creado no puede tener más de $cfg[dbNoticiasTamCreado] caracteres.</p>\n";
    print "\n";
} else {
    $creadoOk = true;
}
